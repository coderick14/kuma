...
class RemoveFilter(html5lib_Filter):
    ...
    def __iter__(self):
        ...
        for token in input:
            ...
            if token['type'] == 'StartTag':
                # move attrs into a dict so we can examine them
                attrs = dict(token['data'])
                # increment counter that tracks nesting
                self.open_level += 1

                # looks in attrs dict to see if this element has the ID we want
                if 'id' in attrs and attrs['id'] is self.section_id:
